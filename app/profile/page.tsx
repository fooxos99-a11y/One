...
function ProfilePage() {
  ...
  const [quranLevel, setQuranLevel] = useState(0)
  ...
  const fetchPlanData = async (studentId: string) => {
    ...
    try {
      ...
      setQuranProgress(data.quranProgressPercent ?? 0)
      setQuranLevel(data.quranLevel ?? Math.round(data.quranProgressPercent ?? 0))
  ...
  const currentQuranLevel = Math.max(0, Math.min(100, quranLevel || 0))
  const currentQuranProgress = Math.max(0, Math.min(100, quranProgress || 0))
  const levelRingRadius = 74
  const levelRingCircumference = 2 * Math.PI * levelRingRadius
  const levelRingOffset = levelRingCircumference * (1 - currentQuranProgress / 100)
      ...
      <div className="min-h-screen flex flex-col bg-gradient-to-br from-[#f5f1e8] to-white">
        ...
        <main className="flex-1 py-6 md:py-12 px-3 md:px-4">
          <div className="container mx-auto max-w-6xl">
            <div className="mb-3 overflow-hidden rounded-[2rem] border border-[#dce6f7] bg-[radial-gradient(circle_at_top_right,_rgba(79,115,209,0.14),_transparent_28%),linear-gradient(135deg,#fbfdff_0%,#ffffff_46%,#f8f2e8_100%)] shadow-[0_26px_60px_rgba(52,83,167,0.10)] md:mb-6">
              <div className="grid gap-8 px-5 py-6 md:grid-cols-[280px_minmax(0,1fr)] md:px-8 md:py-7">
                <div className="flex items-center justify-center">
                  <div className="relative flex h-[210px] w-[210px] items-center justify-center">
                    <div className="absolute inset-0 rounded-full bg-[radial-gradient(circle,_rgba(52,83,167,0.12)_0%,_rgba(52,83,167,0.04)_46%,_transparent_72%)] blur-xl" />
                    <svg viewBox="0 0 200 200" className="absolute inset-0 h-full w-full -rotate-90 drop-shadow-[0_16px_28px_rgba(52,83,167,0.16)]" aria-hidden="true">
                      <circle cx="100" cy="100" r={levelRingRadius} fill="none" stroke="rgba(223,232,247,0.95)" strokeWidth="24" />
                      <circle
                        cx="100"
                        cy="100"
                        r={levelRingRadius}
                        fill="none"
                        stroke="url(#profileLevelRing)"
                        strokeWidth="24"
                        strokeLinecap="round"
                      ...
                      />
                      <defs>
                        <linearGradient id="profileLevelRing" x1="0%" y1="0%" x2="100%" y2="100%">
                          <stop offset="0%" stopColor="#8eb4ff" />
                          <stop offset="45%" stopColor="#5f87e8" />
                          <stop offset="100%" stopColor="#3453a7" />
                        </linearGradient>
                      </defs>
                    </svg>

                    <div className="relative flex h-[162px] w-[162px] items-center justify-center rounded-full border border-white/80 bg-[radial-gradient(circle_at_top,_#ffffff_0%,_#f6f9ff_54%,_#eef3fb_100%)] shadow-[inset_0_1px_0_rgba(255,255,255,0.95),0_18px_40px_rgba(52,83,167,0.10)]">
                      <div className="text-center">
                        <div className="text-7xl font-black leading-none tracking-[-0.04em] text-[#3453a7]">{currentQuranLevel}</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6 text-right">
                  <div className="space-y-3">
                    <div className="space-y-2 rounded-[1.4rem] border border-[#dde7f7] bg-white/80 px-4 py-4 shadow-[0_14px_34px_rgba(15,23,42,0.04)]">
                      <div className="flex items-center justify-between gap-3 text-sm font-extrabold text-[#526071]">
                        <span className="text-[#3453a7]">نسبة التقدم</span>
                        <span>{formatPercent(currentQuranProgress)}</span>
                      </div>
                      <div className="h-3 overflow-hidden rounded-full bg-[#e8eef8]">
                        <div
                          className="h-full rounded-full transition-all duration-700"
                          style={{
                            width: `${currentQuranProgress}%`,
                            background: "linear-gradient(90deg, #9ec0ff 0%, #5f87e8 45%, #3453a7 100%)",
                            boxShadow: "0 6px 18px rgba(52,83,167,0.24)",
                          }}
                        />
                      </div>
                    </div>
                  ...
                  <div className="grid gap-3 sm:grid-cols-3">
                    <div className="group overflow-hidden rounded-[1.4rem] border border-[#d9e3f5] bg-white/90 px-4 py-4 shadow-[0_12px_30px_rgba(15,23,42,0.05)] transition-transform hover:-translate-y-0.5">
                      <div className="text-xs font-extrabold tracking-wide text-[#8b97aa]">النقاط</div>
                      <div className="mt-2 text-3xl font-black leading-none text-[#1a2332]">{studentData.points ?? 0}</div>
                    </div>
                    <div className="group overflow-hidden rounded-[1.4rem] border border-[#d9e3f5] bg-white/90 px-4 py-4 shadow-[0_12px_30px_rgba(15,23,42,0.05)] transition-transform hover:-translate-y-0.5">
                      <div className="text-xs font-extrabold tracking-wide text-[#8b97aa]">الترتيب العام</div>
                      <div className="mt-2 text-3xl font-black leading-none text-[#1a2332]">{rankingData?.globalRank ?? "-"}</div>
                    </div>
                    <div className="group overflow-hidden rounded-[1.4rem] border border-[#d9e3f5] bg-white/90 px-4 py-4 shadow-[0_12px_30px_rgba(15,23,42,0.05)] transition-transform hover:-translate-y-0.5">
                      <div className="text-xs font-extrabold tracking-wide text-[#8b97aa]">ترتيب الحلقة</div>
                      <div className="mt-2 text-3xl font-black leading-none text-[#1a2332]">{rankingData?.circleRank ?? "-"}</div>
                    </div>
                  </div>
                </div>
              </div>
