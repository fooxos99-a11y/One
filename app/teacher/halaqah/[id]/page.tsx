...
export default function HalaqahManagement() {
	...
	useEffect(() => {
		const bootstrapAuth = async () => {
			try {
				...
				const fallbackTeacherData: TeacherSessionFallback | null = data.user
					? {
						id: String(data.user.id || "").trim(),
						name: String(data.user.name || localStorage.getItem("userName") || "").trim(),
						accountNumber: String(data.user.accountNumber || accountNumber || "").trim(),
						halaqah: String(data.user.halaqah || localStorage.getItem("userHalaqah") || "").trim(),
				...
				fetchTeacherData(String(accountNumber), fallbackTeacherData)
			} catch {
				router.push("/login")
			}
		}
	...
	const applyTeacherFallback = (fallbackTeacherData?: TeacherSessionFallback | null) => {
		if (!fallbackTeacherData?.id || !fallbackTeacherData?.name || !fallbackTeacherData?.accountNumber) {
			setHasCircle(false)
			setIsLoading(false)
			return
		}
		...
		if (fallbackTeacherData.halaqah) {
			setHasCircle(true)
			void fetchStudents(fallbackTeacherData.halaqah)
			return
		}
	...
	const fetchTeacherData = async (accountNumber: string, fallbackTeacherData?: TeacherSessionFallback | null) => {
		try {
			const response = await fetch(`/api/teachers?account_number=${accountNumber}`)
			if (!response.ok) {
				applyTeacherFallback(fallbackTeacherData)
				return
			}
			const data = await response.json()
