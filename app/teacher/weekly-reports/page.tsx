...
function getStoredTeacherHalaqah() {
  try {
    const storedUser = localStorage.getItem("currentUser");
    const parsedUser = storedUser ? JSON.parse(storedUser) : null;
    return String(parsedUser?.halaqah || localStorage.getItem("userHalaqah") || "").trim();
  } catch {
    ...
    async function fetchTeacherCircle() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          teacherHalaqah = String(teacher?.halaqah || teacher?.circle_name || "").trim();
        }
        ...
        setTeacherCircle(teacherHalaqah);
      } catch (caughtError) {
        const teacherHalaqah = getStoredTeacherHalaqah();
        if (teacherHalaqah) {
          setTeacherCircle(teacherHalaqah);
          setError("");
        } else {
