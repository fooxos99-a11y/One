...
export function SemestersContent({
  displayMode = "page",
  onInlineActionsChange,
}: {
  displayMode?: "page" | "inline"
  onInlineActionsChange?: (actions: { openEndSemesterDialog: () => void; hasActiveSemester: boolean }) => void
}) {
  ...
  useEffect(() => {
    ...
    onInlineActionsChange({
      openEndSemesterDialog: () => {
        setNewSemesterName(activeSemester?.name || "")
        setIsEndSemesterDialogOpen(true)
      },
      hasActiveSemester: Boolean(activeSemester),
    })
  ...
  }, [activeSemester, displayMode, onInlineActionsChange])
  ...
  const handleEndSemester = async () => {
    ...
    if (!newSemesterName.trim()) {
      toast({ title: "خطأ", description: "أدخل اسم الفصل بعد أرشفته", variant: "destructive" })
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || "تعذر إنهاء الفصل")
      }
      ...
      setNewSemesterName("")
      toast({ title: "تم", description: `تم إنهاء الفصل وأرشفته باسم ${data.archivedSemesterName || activeSemester.name}` })
    } catch (error) {
      toast({
        title: "خطأ",
        description: error instanceof Error ? error.message : "تعذر إنهاء الفصل",
        variant: "destructive",
      })
    } finally {
  ...
  const content = (
    <div className="mx-auto max-w-7xl space-y-6">
          {displayMode !== "inline" && activeSemester ? (
          ...
          <Dialog open={isEndSemesterDialogOpen} onOpenChange={setIsEndSemesterDialogOpen}>
            <DialogContent className="max-w-md rounded-[28px] border border-[#dbe5f1]" dir="rtl">
              <DialogHeader className="text-right">
                <DialogTitle className="text-right text-xl font-black text-[#1a2332]">إنهاء الفصل</DialogTitle>
              </DialogHeader>
              <div className="space-y-4 text-right">
                <div className="rounded-[20px] border border-[#f1deb0] bg-[#fffaf1] px-4 py-3 text-sm font-semibold text-[#8a6a22]">
                  سيتم أرشفة الفصل النشط الحالي، ثم سيتاح لك بدء فصل جديد من نفس الصفحة.
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-bold text-[#1a2332]">اسم الفصل بعد الأرشفة</label>
                  <Input
                    value={newSemesterName}
                    onChange={(event) => setNewSemesterName(event.target.value)}
              ...
              <DialogFooter className="flex-row-reverse gap-3 sm:justify-start">
                <Button type="button" variant="outline" onClick={() => setIsEndSemesterDialogOpen(false)} className="border-[#d8e4fb] text-[#3453a7] hover:bg-[#f5f8ff]">
                  إلغاء
                </Button>
                <Button type="button" onClick={handleEndSemester} disabled={isEndingSemester} className="bg-[#3453a7] text-white hover:bg-[#24428f]">
                  {isEndingSemester ? "جاري الإنهاء..." : "إنهاء الفصل"}
                </Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
  ...
  if (isEmbedded) {
    return <div className="min-h-full bg-white px-4 py-8 md:px-6 md:py-10" dir="rtl">{content}</div>
  }
...
export default function AdminSemestersPage() {
  return <SemestersContent displayMode="page" />
}