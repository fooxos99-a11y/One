...
export default function StudentPlansPage() {
  return <StudentPlansContent displayMode="page" />;
}

export function StudentPlansContent({
  displayMode = "page",
  onInlineActionsChange,
}: {
  displayMode?: "page" | "inline"
  onInlineActionsChange?: (actions: { canResetStudent: boolean; openResetDialog: () => void }) => void
}) {
  ...
  useEffect(() => {
    ...
    onInlineActionsChange({
      canResetStudent: Boolean(selectedCircle && !isCircleDataLoading && students.length > 0),
      openResetDialog: () => {
  ...
  }, [onInlineActionsChange, selectedCircle, isCircleDataLoading, students.length]);
  ...
  if (authLoading || !authVerified) {
    return displayMode === "inline"
      ? <div className="flex min-h-[320px] items-center justify-center bg-[#fafaf9]"><SiteLoader size="md" /></div>
          ...
          <div className="rounded-[1.9rem] border border-[#edf2fb] bg-white px-5 py-5 shadow-[0_20px_60px_rgba(15,23,42,0.06)] md:px-7 md:py-7">
            <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
              <div className="w-full max-w-sm space-y-1.5">
                ...
                {isCirclesLoading ? (
                  ...
                  <div className="flex h-14 items-center rounded-[1.25rem] border border-[#cfdcf4] bg-white px-5 text-sm text-[#8b97aa]">
                    لا توجد حلقات
                  </div>
                ) : (
                  <Select
              ...
              {displayMode !== "inline" && selectedCircle && !isCircleDataLoading && students.length > 0 ? (
                <button
                  onClick={openResetDialog}
                  className="inline-flex h-11 items-center justify-center rounded-full border border-red-200 bg-white px-5 text-sm font-bold text-red-600 shadow-[0_10px_30px_rgba(220,38,38,0.08)] transition-all hover:bg-red-50"
                >
                  إعادة حفظ طالب
                </button>
              ) : null}
            ...
            <div className="mt-6 border-t border-[#dfe7f5]/70 pt-6">
              {isCirclesLoading ? (
                ...
                <div className="flex min-h-[220px] items-center justify-center rounded-[1.5rem] bg-[#fbfdff] px-6 text-center text-lg text-[#8b97aa]">
                  لا توجد حلقات
                </div>
              ) : !selectedCircle ? (
                <div className="flex min-h-[220px] items-center justify-center rounded-[1.5rem] bg-[#fbfdff] px-6 text-center text-lg text-[#8b97aa]">اختر الحلقة لعرض الطلاب</div>
              ) : isCircleDataLoading ? (
                <div className="flex min-h-[220px] items-center justify-center rounded-[1.5rem] bg-[#fbfdff]">
                  <SiteLoader size="lg" />
                </div>
