...
export function FinanceContent({ displayMode = "page" }: { displayMode?: "page" | "inline" }) {
  ...
  const loadFinanceData = async () => {
    ...
    if (!response.ok) {
      throw new Error(data.error || "تعذر تحميل بيانات المالية")
    }

    setActiveSemesterId(String(data.activeSemesterId || ""))
    setInvoices((Array.isArray(data.invoices) ? data.invoices : []).map((row: InvoiceRow) => mapInvoiceRow(row)))
    setExpenses((Array.isArray(data.expenses) ? data.expenses : []).map((row: ExpenseRow) => mapExpenseRow(row)))
    setIncomes((Array.isArray(data.incomes) ? data.incomes : []).map((row: IncomeRow) => mapIncomeRow(row)))
    setTrips((Array.isArray(data.trips) ? data.trips : []).map((row: TripRow) => mapTripRow(row)))
  }
  ...
  const handleDeleteRecord = async (section: EditableSection, recordId: string) => {
    const response = await fetch("/api/finance", {
      ...
      body: JSON.stringify({ section, recordId }),
    })

    const data = await response.json().catch(() => ({}))
    if (!response.ok) {
      const title = section === "invoices" ? "تعذر حذف الفاتورة" : section === "expenses" ? "تعذر حذف المصروف" : section === "incomes" ? "تعذر حذف الإيراد" : "تعذر حذف الرحلة أو الفعالية"
      toast({ title, description: data.error || "فشل الحذف", variant: "destructive" })
      return
    }
  ...
  const handleAddRecord = async () => {
    ...
    if (dialogSection === "invoices") {
      ...
      if (!response.ok) {
        toast({ title: "تعذر حفظ الفاتورة", description: data.error || "فشل الحفظ", variant: "destructive" })
        return
      }

      setInvoices((current) => [mapInvoiceRow(data.record as InvoiceRow), ...current])
      closeDialog()
      return
    }
    ...
    if (dialogSection === "expenses") {
      ...
      if (!response.ok) {
        toast({ title: "تعذر حفظ المصروف", description: data.error || "فشل الحفظ", variant: "destructive" })
        return
      }

      setExpenses((current) => [mapExpenseRow(data.record as ExpenseRow), ...current])
      closeDialog()
      return
    }
    ...
    if (dialogSection === "trips") {
      ...
      if (!response.ok) {
        toast({ title: "تعذر حفظ الرحلة أو الفعالية", description: data.error || "فشل الحفظ", variant: "destructive" })
        return
      }

      setTrips((current) => [mapTripRow(data.record as TripRow), ...current])
      closeDialog()
      return
    }
    ...
    if (dialogSection === "incomes") {
      ...
      if (!response.ok) {
        toast({ title: "تعذر حفظ الإيراد", description: data.error || "فشل الحفظ", variant: "destructive" })
        return
      }

      setIncomes((current) => [mapIncomeRow(data.record as IncomeRow), ...current])
      closeDialog()
    }
  }
  ...
  if (isEmbedded) {
    return (
      <div className="min-h-full bg-gradient-to-br from-[#f8fbff] via-white to-[#f5f9ff] px-1 py-1 font-cairo" dir="rtl">
...
export default function FinancePage() {
  return <FinanceContent displayMode="page" />
}
