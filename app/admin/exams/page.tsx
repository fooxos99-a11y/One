...
import { BellRing, CalendarDays, ChevronLeft, ChevronRight, CircleAlert, Loader2, MessageSquareText, QrCode, Save, SlidersHorizontal, Trash2 } from "lucide-react"
...
type ExamsInlineActions = {
  openSchedulesOverview: () => void
...
export function ExamsContent({
  ...
  onInlineActionsChange?: (actions: ExamsInlineActions) => void
}) {
  ...
  useEffect(() => {
    ...
    onInlineActionsChange({
      openSchedulesOverview: () => setIsSchedulesOverviewOpen(true),
      openExamResults: () => setIsExamResultsOpen(true),
  ...
  if (isLoading || authLoading || !authVerified) {
    return (
      <div className={`${isEmbedded ? "min-h-[60vh]" : "min-h-screen"} flex items-center justify-center bg-white`}>
        <SiteLoader size="lg" />
      </div>
    )
  }
      ...
      <main className="px-4 py-8 md:px-6 md:py-10">
        <div className="mx-auto max-w-7xl space-y-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            {displayMode !== "inline" ? (
            <div className="flex flex-col items-stretch justify-start gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-end">
              <Button
                type="button"
                variant="outline"
                onClick={() => router.push(isEmbedded ? "/admin/whatsapp-qr?embedded=1" : "/admin/whatsapp-qr")}
                className="h-11 w-full rounded-2xl border-[#d7e3f2] bg-white px-5 text-sm font-black text-[#3453a7] shadow-none hover:bg-[#f8fbff] sm:w-auto"
              >
                <span className="relative me-2 inline-flex h-4 w-4 items-center justify-center">
                  <QrCode className="h-4 w-4" />
                  {!isWhatsAppStatusLoading && !isWhatsAppReady ? (
                    <span className="absolute -right-1.5 -top-1.5 inline-flex h-4 w-4 items-center justify-center rounded-full bg-[#dc2626] text-[11px] font-black leading-none text-white">
                      !
                    </span>
                  ) : null}
                </span>
              ...
              <Button type="button" onClick={() => setIsSchedulesOverviewOpen(true)} className="h-11 w-full rounded-2xl border border-[#3453a7] bg-[#3453a7] px-6 text-sm font-black text-white shadow-none hover:bg-[#2c478f] sm:w-auto">
              <CalendarDays className="me-2 h-4 w-4" />
              المواعيد
              </Button>

              <Button type="button" onClick={() => setIsExamResultsOpen(true)} className="h-11 w-full rounded-2xl border border-[#3453a7] bg-[#3453a7] px-6 text-sm font-black text-white shadow-none hover:bg-[#2c478f] sm:w-auto">
              <BellRing className="me-2 h-4 w-4" />
              نتائج الاختبارات
              </Button>

                <Button type="button" onClick={() => setIsTemplatesDialogOpen(true)} className="h-11 w-full rounded-2xl border border-[#3453a7] bg-[#3453a7] px-6 text-sm font-black text-white shadow-none hover:bg-[#2c478f] sm:w-auto">
                <MessageSquareText className="me-2 h-4 w-4" />
                القوالب
                </Button>

              <Button type="button" onClick={() => setIsSettingsOpen(true)} className="h-11 w-full rounded-2xl border border-[#3453a7] bg-[#3453a7] px-6 text-sm font-black text-white shadow-none hover:bg-[#2c478f] sm:w-auto">
...
export default function AdminExamsPage() {
  return <ExamsContent displayMode="page" />
}
