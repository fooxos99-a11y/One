...
import { AlertTriangle, CheckCircle2, LogOut, Play, QrCode, RefreshCw, Smartphone } from "lucide-react"
...
function getStatusUi(status: WhatsAppStatusResponse, isStartingWorker: boolean, startupError: string | null) {
  ...
  if (!status.workerOnline) {
    return {
      ...
      tone: "bg-rose-50 text-rose-700 border-rose-200",
      description: "العامل غير متصل حالياً. سيحاول النظام تشغيل العامل المحلي تلقائياً، وإذا لم يبدأ فراجع ملف .env.local-worker.",
    }
  }
            ...
            <div className="flex items-center gap-2">
              {!status.workerOnline ? (
                <Button
                  ...
                  className="h-11 w-11 rounded-2xl bg-[#3453a7] text-white hover:bg-[#2c478d] disabled:opacity-60"
                >
