...
function resolveWorkerPath(configuredPath, fallbackPath) {
...
function buildOutgoingMedia(row) {
  ...
  if (!mimeType || !mediaBase64) {
    throw new Error(`Outgoing media payload for message ${row?.id || "unknown"} is incomplete.`)
  }
...
function currentWorkerOwnsLock() {
  if (!hasWorkerLock) {
    return true
  }
...
function getQueueRowTimestamp(row) {
  const createdAtTime = row?.created_at ? new Date(row.created_at).getTime() : Number.NaN
  return Number.isFinite(createdAtTime) ? createdAtTime : Number.MAX_SAFE_INTEGER
}

function compareQueuedRows(firstRow, secondRow) {
  const firstTimestamp = getQueueRowTimestamp(firstRow)
  const secondTimestamp = getQueueRowTimestamp(secondRow)
    ...
    try {
      ...
      const sentMessage = outgoingMedia
        ? messageType === "document"
          ? await whatsappClient.sendMessage(chatId, outgoingMedia.media, {
...
async function loadPendingMessages() {
  const { data, error } = await supabase
    .from(QUEUE_TABLE)
    .select("id, phone_number, message, message_type, media_mime_type, media_base64, media_file_name, status, created_at")    .eq("status", "pending")
...
async function skipPendingMessagesAfterReconnect() {
  ...
  log(`Skipped ${pendingIds.length} stale queued message(s) after reconnect.`)}
